...
target <- my_flights %>% filter(origin == "JFK" & dest == "SEA") %>% na.omit(cols=c(airt_itme, dep_time)) %>% summarize(avg_time=mean(air_time), min_time=min(air_time), max_time=max(air_time))
target
...
target <- my_flights %>% filter(dest == "HOU" | dest == "IAH") %>% 
